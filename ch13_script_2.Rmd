...
Sentence: "If we divide the US population into 5 income categories and compare people in two adjoining buckets, say the 3s and the 4s, then the richer people are about 5% less likely to prefer the Democratic candidate, all else equal, but it might be as low as 4% or as high as 7%."
...
```{r}
nes_preds <- nes %>%
  mutate(logistic = predict(logistic_fit, new_data = nes) %>% pull(.pred_class),
         cart = predict(cart_fit, new_data = nes) %>% pull(.pred_class),
         rf = predict(rf_fit, new_data = nes) %>% pull(.pred_class))
nes_preds %>%
  filter(! (cart == rf)) %>% 
  select(gender, income, vote, logistic, cart, rf) %>%
  sample_n(5)
```

```{r}
tibble(logistic = mean(nes_preds$logistic == nes_preds$vote),
       tree = mean(nes_preds$cart == nes_preds$vote),
       forest = mean(nes_preds$rf == nes_preds$vote))
```
